...
;var $j=jQuery.noConflict();$j(document).on('ready',function(){"use strict";oceanwpVerticalHeader()});function oceanwpVerticalHeader(){"use strict"
...
return!1});if($siteHeader.length&&!navigator.userAgent.match(/(Android|iPod|iPhone|iPad|IEMobile|Opera Mini)/)){$siteHeader.niceScroll({autohidemode:!1,cursorborder:0,cursorborderradius:0,cursorcolor:'transparent',cursorwidth:0,horizrailenabled:!1,mousescrollstep:40,scrollspeed:60,zindex:100005,})}
...
listener.on('keyup blur',function(){if(listener.val().length>0){form.addClass('search-filled')}else{form.removeClass('search-filled')}})})}var $j=jQuery.noConflict(),$window=$j(window);$j(document).on('ready',function(){"use strict";oceanwpScrollEffect()});function oceanwpScrollEffect(){"use strict"
...
$window.on('scroll',function(){if($j(this).scrollTop()>100){$j('#scroll-top').fadeIn()}else{$j('#scroll-top').fadeOut()}});$j.each(selectors,function(key,value){$j(value).on('click',function(e){e.preventDefault();$j('html, body').animate({scrollTop:0},400);$j(this).parent().removeClass('sfHover')})})}var $j=jQuery.noConflict(),$window=$j(window);$j(document).on('ready',function(){"use strict";oceanwpFullScreenMobile()});function oceanwpFullScreenMobile(){"use strict"
...
$mobileLink.on('click',function(){$j(this).addClass('exit');$mobileMenu.addClass('active').fadeIn(200);$j('.mobile-menu > .hamburger').addClass('is-active');var innerWidth=$j('html').innerWidth();$j('html').css('overflow','hidden');var hiddenInnerWidth=$j('html').innerWidth();$j('html').css('margin-right',hiddenInnerWidth-innerWidth);return!1});$j('#mobile-fullscreen .menu-item-has-children').children('a').append('<span class="dropdown-toggle"></span>');$j('#mobile-fullscreen nav ul > li.menu-item-has-children > a').on('tap click',function(){if($j(this).closest('li.menu-item-has-children').find('> ul.sub-menu').is(':visible')){$j(this).closest('li.menu-item-has-children').removeClass('open-sub');$j(this).closest('li.menu-item-has-children').find('> ul.sub-menu').slideUp(200)}else{$j(this).closest('li.menu-item-has-children').addClass('open-sub');$j(this).closest('li.menu-item-has-children').find('> ul.sub-menu').slideDown(200)}
return!1});$j('#mobile-fullscreen a.close').on('click',function(e){e.preventDefault();oceanwpFullScreenMobileClose()});$j('#mobile-fullscreen .fs-dropdown-menu li a[href*="#"]:not([href="#"]), #mobile-fullscreen #mobile-nav li a[href*="#"]:not([href="#"])').on('click',function(){oceanwpFullScreenMobileClose()});$window.resize(function(){if($window.width()>=960){oceanwpFullScreenMobileClose()}})}}
